...
const ReceiptScanner = ({ isOpen, onClose, onScanComplete }) => {
  ...
  // 실제 OCR API 호출 함수
  const processReceiptOcr = useCallback(
    async (imageData) => {
      try {
  ...
  const simulateLoading = useCallback(
    ...
    [onScanComplete, processReceiptOcr]
  );
  ...
  // 로딩 상태 메시지 결정
  const getLoadingStatusMessage = useCallback(() => {
  ...
  // 모든 훅이 무조건적으로 호출되도록 보장
  const children = useMemo(() => {
    // 포털 렌더링이 불가능한 경우 null 반환
    if (!shouldRenderPortal) return null;

    return (
      <div className="receipt-scanner-fullscreen">
        <motion.div
  ...
  // 컴포넌트의 모든 렌더링에서 동일한 순서로 훅 호출 보장
  return shouldRenderPortal && portalElement
